...
- **图片格式选择**：支持导出 WebP、PNG、JPG 多种格式
- **拖拽上传**：支持拖拽图片到画布区域，自动识别为背景图或图标
- **图标搜索优化**：输入框失焦或按回车后才触发搜索，避免输入过程中频繁弹窗
- **标题水印改为占位符**：默认标题和水印以 placeholder 形式显示，不预填内容
- **统计脚本可配置**：Umami 统计通过环境变量配置，未配置则不加载
- **ICP 备案号支持**：通过环境变量 `VITE_APP_ICP_NUMBER` 配置，自动显示在页脚
- **密码输入框**：添加显示/隐藏切换按钮
...
| `VITE_APP_FONT_FAMILY` | 全局字体名称（选填），可用值见下方 | `MaokenZhuyuanTi` |
| `VITE_APP_FONT_CSS_URL` | 全局字体 CSS 文件地址（选填），配合 `VITE_APP_FONT_FAMILY` 使用 | `https://chinese-fonts-cdn.deno.dev/packages/mkzyt/dist/猫啃珠圆体/result.css` |
